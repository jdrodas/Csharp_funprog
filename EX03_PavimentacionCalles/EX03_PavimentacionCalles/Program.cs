    ...
    public class Program
    {
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i<lasCalles.Length; i++)
            {
                ...
                lasCalles[i].TramoAfectado = aleatorio.Next(20, 60);
            }
            ...
            for (int i = 0; i < lasCalles.Length; i++)
            {
                Console.WriteLine($"\nCalle No. {(i + 1)}: \nDeterioro: {lasCalles[i].TipoDeterioro}, " +
                    ...
                    $"tramo afectado: {lasCalles[i].TramoAfectado}% ");
            }

            //Este procedimiento se puede colocar en una función. ¿Cómo lo harías?
            int[] totalAfectacionesPorDeterioro = TotalizaAfectacionesPorDeterioro(lasCalles, losDeterioros);

            float[] cantidadPavimento = ObtieneCantidadCallePavimentadaPorDeterioro(lasCalles, losDeterioros);

            float[] longitudesPromedio = ObtieneLongitudPromedioTramosPorDeterioro(lasCalles, losDeterioros);
            ...
            for (int i = 0; i < losDeterioros.Length; i++)
            {
                Console.WriteLine($"\n{losDeterioros[i]}:\n " +
                    ...
                    $"Total mts pavimentados: {cantidadPavimento[i].ToString(".00")}, " +
                    $"para una longitud promedio de {longitudesPromedio[i].ToString("0.00")} mts\n");
            }
        }

        static float[] ObtieneLongitudPromedioTramosPorDeterioro(Calle[] lasCalles, string[] losDeterioros)
        {
        ...
        static float[] ObtieneCantidadCallePavimentadaPorDeterioro(Calle[] lasCalles, string[] losDeterioros)
        {
            float[] cantidades = new float[losDeterioros.Length];
